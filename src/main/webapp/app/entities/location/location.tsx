  ...
  return (
    <div>
      <h2 id="location-heading" data-cy="LocationHeading">
        <Translate contentKey="meApplicationApp.location.home.title">Locations</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="meApplicationApp.location.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/location/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="meApplicationApp.location.home.createLabel">Create new Location</Translate>
          </Link>
        </div>
      </h2>
        ...
        {locationList && locationList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="meApplicationApp.location.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('streetAddress')}>
                  <Translate contentKey="meApplicationApp.location.streetAddress">Street Address</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('streetAddress')} />
                </th>
                <th className="hand" onClick={sort('postalCode')}>
                  <Translate contentKey="meApplicationApp.location.postalCode">Postal Code</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('postalCode')} />
                </th>
                <th className="hand" onClick={sort('city')}>
                  <Translate contentKey="meApplicationApp.location.city">City</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('city')} />
                </th>
                <th className="hand" onClick={sort('stateProvince')}>
                  <Translate contentKey="meApplicationApp.location.stateProvince">State Province</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('stateProvince')} />
                </th>
                <th>
                  <Translate contentKey="meApplicationApp.location.country">Country</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="meApplicationApp.location.home.notFound">No Locations found</Translate>
            </div>
          )
        )}
      </div>
